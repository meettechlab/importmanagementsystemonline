...
class CrusherStockSScreen extends StatefulWidget {
  const CrusherStockSScreen({Key? key}) : super(key: key);

  @override
  _CrusherStockSScreenState createState() => _CrusherStockSScreenState();
}

class _CrusherStockSScreenState extends State<CrusherStockSScreen> {
      ...
      for (var doc in querySnapshot.docs) {
        if (doc["port"].toLowerCase() == ("shutarkandi")) {
          setState(() {
            ...
            _totalPurchase = (_totalPurchase + double.parse(doc["price"])).floor();
          });
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text('Crusher Stock ( Shutarkandi )'),
        actions: [
          TextButton(
              onPressed: (){
                Navigator.push(
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  "Total Purchase : $_totalPurchase TK",
                  style: TextStyle(color: Colors.red, fontSize: 18),
                ),
              ],
            ),
                    ...
                    FirebaseFirestore.instance
                      ...
                      for (var doc in querySnapshot.docs) {
                        if (doc["invoice"] == cStock["invoice"] &&
                            doc["port"].toLowerCase() == ("shutarkandi")) {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) =>
                ...
                IconButton(
                  onPressed: () {
                    ...
                    FirebaseFirestore.instance
                        ...
                        .then((QuerySnapshot querySnapshot) {
                      for (var doc in querySnapshot.docs) {
                        if( doc["id"] ==  "crusherstockshutarkandi" + cStock["invoice"]){
                          setState(() {
                            doc.reference.delete();
                          });
                        }
